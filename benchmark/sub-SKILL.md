...
- → See `performance/sub-SKILL.md` skill for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `troubleshooting/sub-SKILL.md` skill for pprof setup on running services (enable, secure, capture), Delve debugger, GODEBUG flags, root cause methodology
- → See `observability/sub-SKILL.md` skill for everyday always-on monitoring, continuous profiling (Pyroscope), distributed tracing (OpenTelemetry)
- → See `testing/sub-SKILL.md` skill for general testing practices
