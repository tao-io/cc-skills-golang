...
> **Community default.** A company skill that explicitly supersedes `context/sub-SKILL.md` skill takes precedence.
...
- → See the `concurrency/sub-SKILL.md` skill for goroutine cancellation patterns using context
- → See the `database/sub-SKILL.md` skill for context-aware database operations (QueryContext, ExecContext)
- → See the `observability/sub-SKILL.md` skill for trace context propagation with OpenTelemetry
- → See the `design-patterns/sub-SKILL.md` skill for timeout and resilience patterns

## Enforce with Linters

Many context pitfalls are caught automatically by linters: `govet`, `staticcheck`. → See the `lint/sub-SKILL.md` skill for configuration and usage.
