...
Built-in and standard library data structures: internals, correct usage, and selection guidance. For safety pitfalls (nil maps, append aliasing, defensive copies) see `safety/sub-SKILL.md` skill. For channels and sync primitives see `concurrency/sub-SKILL.md` skill. For string/byte/rune choice see `design-patterns/sub-SKILL.md` skill.
...
A slice is a 3-word header: pointer, length, capacity. Multiple slices can share a backing array (→ see `safety/sub-SKILL.md` for aliasing traps and the header diagram).
...
Key functions: `Sort`/`SortFunc`, `BinarySearch`, `Contains`, `Compact`, `Grow`. For `Clone`, `Equal`, `DeleteFunc` → see `safety/sub-SKILL.md` skill.
...
For `Clone`, `Equal`, sorted iteration → see `safety/sub-SKILL.md` skill.
...
- → See `performance/sub-SKILL.md` skill for struct field alignment, memory layout optimization, and cache locality
- → See `safety/sub-SKILL.md` skill for nil map/slice pitfalls, append aliasing, defensive copying, `slices.Clone`/`Equal`
- → See `concurrency/sub-SKILL.md` skill for channels, `sync.Map`, `sync.Pool`, and all sync primitives
- → See `design-patterns/sub-SKILL.md` skill for `string` vs `[]byte` vs `[]rune`, iterators, streaming
- → See `structs-interfaces/sub-SKILL.md` skill for struct composition, embedding, and generics vs `any`
- → See `code-style/sub-SKILL.md` skill for slice/map initialization style
