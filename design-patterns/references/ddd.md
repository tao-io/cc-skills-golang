...
For large systems, each context can be its own Go module in a workspace (`go.work`). See the `project-layout/sub-SKILL.md` skill for workspace setup.

## Wiring

Wire dependencies in `cmd/server/main.go` using manual constructor injection. → See `dependency-injection/sub-SKILL.md` skill for DI library alternatives.
