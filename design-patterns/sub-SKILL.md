...
> **Community default.** A company skill that explicitly supersedes `design-patterns/sub-SKILL.md` skill takes precedence.

# Go Design Patterns & Idioms

Idiomatic Go patterns for production-ready code. For error handling details see the `error-handling/sub-SKILL.md` skill; for context propagation see `context/sub-SKILL.md` skill; for struct/interface design see `structs-interfaces/sub-SKILL.md` skill.
...
12. **Use `strings.Builder`** for concatenation in loops → see `code-style/sub-SKILL.md`
...
17. **Use `crypto/rand`** for keys/tokens — `math/rand` is predictable → see `security/sub-SKILL.md`
...
→ See `structs-interfaces/sub-SKILL.md` for the `var _ Interface = (*Type)(nil)` pattern.

## Error Flow Patterns

Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `code-style/sub-SKILL.md` for the full pattern and examples.
...
Retry logic MUST check `ctx.Err()` between attempts and use exponential/linear backoff via `select` on `ctx.Done()`. Long loops MUST check `ctx.Err()` periodically. → See `context/sub-SKILL.md` skill.

## Database Patterns

→ See `database/sub-SKILL.md` skill for sqlx/pgx, transactions, nullable columns, connection pools, repository interfaces, testing.
...
- **Respect 12-factor app** principles — → see `project-layout/sub-SKILL.md`
...
- → See `data-structures/sub-SKILL.md` skill for data structure selection, internals, and container/ packages
- → See `error-handling/sub-SKILL.md` skill for error wrapping, sentinel errors, and the single handling rule
- → See `structs-interfaces/sub-SKILL.md` skill for interface design and composition
- → See `concurrency/sub-SKILL.md` skill for goroutine lifecycle and graceful shutdown
- → See `context/sub-SKILL.md` skill for timeout and cancellation patterns
- → See `project-layout/sub-SKILL.md` skill for architecture and directory structure
