...
> **Community default.** A company skill that explicitly supersedes `error-handling/sub-SKILL.md` skill takes precedence.
...
- → See `samber-oops/sub-SKILL.md` for full samber/oops API, builder patterns, and logger integration
- → See `observability/sub-SKILL.md` for structured logging setup, log levels, and request logging middleware
- → See `safety/sub-SKILL.md` for nil interface trap and nil error comparison pitfalls
- → See `naming/sub-SKILL.md` for error naming conventions (ErrNotFound, PathError)
