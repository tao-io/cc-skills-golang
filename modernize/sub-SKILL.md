...
The `modernize` linter (available since **golangci-lint v2.6.0**) automatically detects code that can be rewritten using newer Go features. It originates from `golang.org/x/tools/go/analysis/passes/modernize`; `gopls` and Go 1.26's rewritten `go fix` cover overlapping modernization checks, but exact coverage differs by tool version. See the `lint/sub-SKILL.md` skill for configuration.
...
26. Set up AI-driven code review in CI — loads these skills to guide review per area; see `continuous-integration/sub-SKILL.md`

## Related Skills

See `concurrency/sub-SKILL.md`, `testing/sub-SKILL.md`, `observability/sub-SKILL.md`, `error-handling/sub-SKILL.md`, `lint/sub-SKILL.md`, `continuous-integration/sub-SKILL.md` skills.
