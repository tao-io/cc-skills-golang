...
> **Community default.** A company skill that explicitly supersedes `observability/sub-SKILL.md` skill takes precedence.
...
See `error-handling/sub-SKILL.md` skill for the single handling rule. See `troubleshooting/sub-SKILL.md` skill for using observability signals to diagnose production issues. See `security/sub-SKILL.md` skill for protecting pprof endpoints and avoiding PII in logs. See `context/sub-SKILL.md` skill for propagating trace context across service boundaries. See `samber/cc-skills@promql-cli` skill for querying and exploring PromQL expressions against Prometheus from the CLI.
