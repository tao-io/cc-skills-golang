...
1. **Profile before optimizing** — intuition about bottlenecks is wrong ~80% of the time. Use pprof to find actual hot spots (→ See `troubleshooting/sub-SKILL.md` skill)
...
**When external:** optimize that component instead — query tuning, caching, connection pools, circuit breakers (→ See `database/sub-SKILL.md` skill, [Caching Patterns](references/caching.md)).
...
2. **Write an atomic benchmark** — isolate one function per benchmark to avoid result contamination (→ See `benchmark/sub-SKILL.md` skill)
...
| Lock contention | mutex/block profile hot | → See `concurrency/sub-SKILL.md` skill |
| Slow queries | DB time dominates traces | → See `database/sub-SKILL.md` skill |
...
Automate benchmark comparison in CI to catch regressions before they reach production. → See `benchmark/sub-SKILL.md` skill for `benchdiff` and `cob` setup.

## Cross-References

- → See `benchmark/sub-SKILL.md` skill for benchmarking methodology, `benchstat`, and `b.Loop()` (Go 1.24+)
- → See `troubleshooting/sub-SKILL.md` skill for pprof workflow, escape analysis diagnostics, and performance debugging
- → See `data-structures/sub-SKILL.md` skill for slice/map preallocation and `strings.Builder`
- → See `concurrency/sub-SKILL.md` skill for worker pools, `sync.Pool` API, goroutine lifecycle, and lock contention
- → See `safety/sub-SKILL.md` skill for defer in loops, slice backing array aliasing
- → See `database/sub-SKILL.md` skill for connection pool tuning and batch processing
- → See `observability/sub-SKILL.md` skill for continuous profiling in production
