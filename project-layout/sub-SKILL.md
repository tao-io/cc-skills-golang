...
→ See `design-patterns/sub-SKILL.md` skill for detailed architecture guides with file trees and code examples.

## Dependency Injection: Ask Next

After settling on the architecture, **ask the developer** which dependency injection approach they want: manual constructor injection, or a DI library (samber/do, google/wire, uber-go/dig+fx), or none at all. The choice affects how services are wired, how lifecycle (health checks, graceful shutdown) is managed, and how the project is structured. See the `dependency-injection/sub-SKILL.md` skill for a full comparison and decision table.
...
Packages MUST be lowercase, singular, and match their directory name. → See `naming/sub-SKILL.md` skill for complete package naming conventions and examples.
...
- **.golangci.yml** — linter config. See the `lint/sub-SKILL.md` skill for the recommended configuration
...
- [ ] **Ask the developer** their preferred DI approach — see `dependency-injection/sub-SKILL.md` skill
...
→ See `cli/sub-SKILL.md` skill for CLI tool structure and Cobra/Viper patterns. → See `dependency-injection/sub-SKILL.md` skill for DI approach comparison and wiring. → See `lint/sub-SKILL.md` skill for golangci-lint configuration. → See `continuous-integration/sub-SKILL.md` skill for CI/CD pipeline setup. → See `design-patterns/sub-SKILL.md` skill for architectural patterns.
