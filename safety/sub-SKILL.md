...
Maps MUST NOT be accessed concurrently — → see `concurrency/sub-SKILL.md` for sync primitives.
...
For integer overflow as a security vulnerability, see the `security/sub-SKILL.md` skill section.
...
→ See `concurrency/sub-SKILL.md` for goroutine lifecycle and leak prevention.
...
→ See `design-patterns/sub-SKILL.md` for why init() should be avoided in favor of explicit constructors.

## Enforce with Linters

Many safety pitfalls are caught automatically by linters: `errcheck`, `forcetypeassert`, `nilerr`, `govet`, `staticcheck`. See the `lint/sub-SKILL.md` skill for configuration and usage.
...
- → See `concurrency/sub-SKILL.md` skill for concurrent access patterns and sync primitives
- → See `data-structures/sub-SKILL.md` skill for slice/map internals, capacity growth, and container/ packages
- → See `error-handling/sub-SKILL.md` skill for nil error interface trap
- → See `security/sub-SKILL.md` skill for security-relevant safety issues (memory safety, integer overflow)
- → See `troubleshooting/sub-SKILL.md` skill for debugging panics and race conditions
...
| Multiple `init()` with ordering assumptions | `init()` execution order across files is unspecified. → See `design-patterns/sub-SKILL.md` — use explicit constructors |
| Blocking forever on nil channel | Nil channels block on both send and receive. Always initialize before use |

## Cross-References

- → See `continuous-integration/sub-SKILL.md` skill for automated AI-driven code review in CI using these guidelines
