...
- → See `samber-lo/sub-SKILL.md` skill for finite slice transforms (Map, Filter, Reduce, GroupBy) — use lo when data is already in a slice
- → See `samber-mo/sub-SKILL.md` skill for monadic types (Option, Result, Either) that compose with ro pipelines
- → See `samber-hot/sub-SKILL.md` skill for in-memory caching (also available as an ro plugin)
- → See `concurrency/sub-SKILL.md` skill for goroutine/channel patterns when reactive streams are overkill
- → See `observability/sub-SKILL.md` skill for monitoring reactive pipelines in production
