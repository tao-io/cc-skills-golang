---
name: golang-testing
description: "Production-ready Golang tests — table-driven tests, testify suites and mocks, parallel tests, fuzzing, fixtures, goroutine leak detection with goleak, snapshot testing, code coverage, integration tests, idiomatic test naming. Use when writing or reviewing Go tests, choosing a testing approach, setting up Go test CI, or debugging flaky/slow tests. For testify-specific APIs see `stretchr-testify/sub-SKILL.md`; for measurement methodology see `benchmark/sub-SKILL.md`."
user-invocable: true
license: MIT
...
> **Community default.** A company skill that explicitly supersedes `testing/sub-SKILL.md` skill takes precedence.
...
→ See `benchmark/sub-SKILL.md` skill for advanced benchmarking: `b.Loop()` (Go 1.24+), `benchstat`, profiling from benchmarks, and CI regression detection.
...
Many test best practices are enforced automatically by linters: `thelper`, `paralleltest`, `testifylint`. See the `lint/sub-SKILL.md` skill for configuration and usage.

## Cross-References

- -> See `stretchr-testify/sub-SKILL.md` skill for detailed testify API (assert, require, mock, suite)
- -> See `database/sub-SKILL.md` skill (testing.md) for database integration test patterns
- -> See `concurrency/sub-SKILL.md` skill for goroutine leak detection with goleak
- -> See `continuous-integration/sub-SKILL.md` skill for CI test configuration and GitHub Actions workflows
- -> See `lint/sub-SKILL.md` skill for testifylint and paralleltest configuration
