...
**Choose fx** for long-running services (HTTP servers, workers, daemons) — lifecycle and signal handling are non-negotiable there. See `uber-fx/sub-SKILL.md` skill.
...
dig has **no built-in lifecycle**. If you need OnStart/OnStop hooks, signal handling, and graceful shutdown, use fx — see `uber-fx/sub-SKILL.md` skill.
...
- → See `uber-fx/sub-SKILL.md` skill for application lifecycle, modules, and signal-aware Run() built on top of dig
