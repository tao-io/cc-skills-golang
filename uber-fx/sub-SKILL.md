...
**Choose raw dig** when you need wiring without a framework: CLI tools, libraries that expose a container to callers, test harnesses, or embedding DI into an existing app that manages its own lifecycle. See `uber-dig/sub-SKILL.md` skill.
...
- → See `uber-dig/sub-SKILL.md` skill for the underlying container, `dig.In`/`dig.Out`, and DI without lifecycle
- → See `dependency-injection/sub-SKILL.md` skill for DI concepts and library comparison
- → See `samber-do/sub-SKILL.md` skill for a generics-based alternative without reflection
- → See `google-wire/sub-SKILL.md` skill for compile-time DI (no runtime container)
- → See `structs-interfaces/sub-SKILL.md` skill for interface design patterns
- → See `context/sub-SKILL.md` skill for context propagation in OnStart/OnStop hooks
- → See `testing/sub-SKILL.md` skill for general testing patterns
